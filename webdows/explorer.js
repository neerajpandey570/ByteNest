	...
	windows : {}, theme : function(themeName, extraCSS) {
		...
		initiate : function() {
			...
			$.each(system.registry.get('HKEY_LOCAL_WEBDOWS/explorer/startmenu'), function() {
				var data = this;
				 if(data.title != "FloydCraft MapApp" ){
					if(data.title == "FloydCraft eMail" ){ data.title = "ByteNest eMail" }

					if(data.title == "FC Minecraft Tools" ){ data.title = "BN Minecraft Tools" }
                    if(data.title == "About FC WebOS" ){ data.title = "About BN WebOS" }
